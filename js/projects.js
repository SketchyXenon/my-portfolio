/* ================================================================
   js/projects.js — Render project cards + tag filter
   
   Security: all data from content.js is inserted via textContent
   or validated safeLink(). No innerHTML used for untrusted data.
   ================================================================ */

import { safeUrl, safeLink, setText, el, safeSvg } from "./security.js";

// ── Static SVG icon builders (our own markup, safe) ───────────
...
export function initProjects() {
  ...
  const allTags = [
    ...
    ...new Set(
      projects.flatMap((p) =>
        Array.isArray(p.tech) ? p.tech.map(String) : [],
      ),
    ),
  ];
  ...
  function render() {
    ...
    const list =
      activeFilter === "All"
        ? projects
        : projects.filter(
            (p) => Array.isArray(p.tech) && p.tech.includes(activeFilter),
          );
